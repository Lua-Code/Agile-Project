...
export default function EditCourse() {
    ...
    const fetchCourse = async () => {
      try {
        ...
        setFormData({
          ...
          professorId: data.professorId?._id || "",
          status: data.status || "active",
          prerequisites: data.prerequisites?.map(p => p.courseCode).join(", ") || "",
        });
        setOriginalCourseCode(data.courseCode || "");
        if (data.professorId?.userId?.fullName) {
          setProfessorSearch(data.professorId.userId.fullName);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (!code || code === originalCourseCode) { setCourseCodeError(null); return; }
  ...
  }, [formData.courseCode, originalCourseCode]);
      ...
      <div style={styles.page}>
        <main style={styles.main}>
          <p style={{ color: "#ef4444", fontSize: "18px" }}>
            Access denied. Only admins can edit courses.
          </p>
        </main>
      </div>
    );
      ...
      <main style={styles.main}>
        <header style={styles.header}>
          <div>
            ...
            <p style={styles.subtitle}>Update the course details below</p>
          </div>
